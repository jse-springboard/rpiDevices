...
def hold(PR,ADC,pressure=2.0,testT=5.0,sampleT=0.5,_settlingTime = 10,ID='Hold'):
    ...
    dataFrame.loc[:,['Time','Pressure (bar)']].to_csv(f'~/springboardRPi/responsePlots/{ID}DataPressure.csv',index=False,header=True)
    dataFrame.loc[:,['Time','Flow rate (ul/min)']].to_csv(f'~/springboardRPi/responsePlots/{ID}DataFlow.csv',index=False,header=True)
